...
# 尝试导入speechbrain，如果失败则使用本地实现
try:
    from speechbrain.inference import EncoderClassifier
    HAS_SPEECHBRAIN = True
except ImportError:
    print("Warning: speechbrain library not installed. Speaker similarity features will be disabled.")
...
MATCHA_CKPT = "/home/i-liyuxin/Depression_TTS/logs/train_daic_utter/decouple_unfreeze_all/runs/2025-09-18_23-41-50/checkpoints/checkpoint_epoch=199.ckpt"
...
OUTPUT_FOLDER_BASE = "/data/depression_tts/synthese_data/v3_dep_slerp_A_1008"
...
SIM_USE_ECAPA_192 = True         # 采样相似度评估使用ECAPA原生192维空间（推荐）
...
DEPRESSION_EXTRACTOR_LAYER = 18     # WavLM 特征层
...
DEPRESSION_EMBEDDINGS_FILE = "/home/i-liyuxin/Contrastive_OS/GDST_embeddings_subject-3-trf/train_embeddings.npz"
SPEAKER_EMBEDDINGS_FILE = "/home/i-liyuxin/Contrastive_OS/speaker_embeddings_subject-3-trf/train_speaker_embeddings.npz"
...
class SpeakerSimilarityModel:
    """冻结的Speaker Verification模型，用于计算speaker similarity"""
    
    def __init__(self, output_dim=32, use_reducer=True):
        """初始化SpeechBrain说话人编码器
        use_reducer: True 则输出经过降维的向量(32维)；False 则直接输出ECAPA原生192维
        """
        if not HAS_SPEECHBRAIN:
            raise ImportError("speechbrain library is not available. Please install it to use speaker similarity features.")
        ...
        try:
            ...
            print("✓ 成功加载SpeechBrain说话人编码器")
            
            self.use_reducer = bool(use_reducer)
            # 添加降维层，将192维转换为32维（与之前提取脚本结构一致）
            if self.use_reducer:
            ...
            # 冻结模型参数
            for param in self.spk_enc.parameters():
                param.requires_grad = False
            if self.use_reducer:
                for param in self.dimension_reducer.parameters():
                    param.requires_grad = False
                
            if self.use_reducer:
                print(f"✓ 添加降维层，将192维转换为{output_dim}维，模型已冻结")
            else:
                print("✓ 相似度空间使用ECAPA原生192维（不做降维）")
            
        except Exception as e:
            print(f"✗ 加载说话人编码器失败: {e}")
            raise
    
    def extract_speaker_embedding(self, waveform, sr=16000):
        """从音频波形提取说话人embedding"""
        try:
            # 确保波形是2D张量 [channels, time]
            if waveform.dim() == 1:
            ...
            # 提取说话人embedding
            with torch.no_grad():
                embeddings = self.spk_enc.encode_batch(waveform)
                # 取平均（如果有多个时间步）
                if embeddings.dim() > 2:
                ...
                if self.use_reducer:
                    # 通过降维层转换为32维
                    embeddings = self.dimension_reducer(embeddings)
                
            return embeddings.cpu().numpy()
            
        except Exception as e:
            print(f"✗ 提取说话人embedding失败: {e}")
            return None

    @torch.inference_mode()
    def extract_raw_ecapa_embedding(self, waveform, sr=16000):
        """直接返回ECAPA原生192维（不经过降维层）。"""
        try:
            if waveform.dim() == 1:
                waveform = waveform.unsqueeze(0)
            elif waveform.dim() == 3:
            ...
            if sr != 16000:
                ...
                waveform = resampler(waveform)
            waveform = waveform.to(device)
            emb = self.spk_enc.encode_batch(waveform)
            if emb.dim() > 2:
                emb = torch.mean(emb, dim=1)
            return emb.cpu().numpy()
        except Exception as e:
            print(f"✗ 提取原生ECAPA embedding失败: {e}")
            return None
    
    def compute_similarity(self, embedding1, embedding2):
        """计算两个speaker embedding之间的余弦相似度"""
        try:
...
def load_personachat_dataset():
    ...
    return train_data
# ───────────────────────────  ECAPA参考嵌入构建  ───────────────────────────────────
def build_ecapa_reference_embeddings(sim_model: SpeakerSimilarityModel, subject_ids: set[int], audio_dir: str, max_audio_per_subject: int = 5) -> dict[int, np.ndarray]:
    """为给定subjects从原始训练集构建ECAPA(192维)参考嵌入。
    策略：每个subject在audio_dir中选取前N条匹配 {sid}_*.wav 的音频，提取原生ECAPA嵌入并取均值。
    返回：{subject_id: embedding(192,)}
    """
    refs: dict[int, np.ndarray] = {}
    if not os.path.isdir(audio_dir):
        print(f"Warning: 参考音频目录不存在: {audio_dir}")
        return refs
    ...
    for sid in sorted(subject_ids):
        try:
            ...
            # 取前N条音频（最多max_audio_per_subject条）
            selected_audios = sorted(candidates)[:max_audio_per_subject]
            embeddings = []
            
            for audio_file in selected_audios:
                audio_path = os.path.join(audio_dir, audio_file)
                wav, sr = torchaudio.load(audio_path)
                emb = sim_model.extract_raw_ecapa_embedding(wav, sr=sr)
                if emb is None:
                    continue
                if isinstance(emb, np.ndarray):
                    e = emb.squeeze()
                else:
                    e = np.array(emb).squeeze()
                embeddings.append(e)
            
            if embeddings:
                # 取多条音频嵌入的均值
                ref_embedding = np.mean(embeddings, axis=0)
                refs[int(sid)] = ref_embedding
                print(f"  ✓ Subject {sid}: 使用 {len(embeddings)} 条音频构建参考嵌入")
        except Exception as e:
            print(f"  ✗ Subject {sid}: 构建参考嵌入失败 - {e}")
            continue

    print(f"✓ 构建ECAPA参考嵌入完成：{len(refs)}/{len(subject_ids)} subjects")
    return refs
...
@torch.inference_mode()
def synthesise_with_conditions(model: MatchaTTS, text: str, depression_embedding, speaker_embedding, temperature_override=None):
...
@torch.inference_mode()
def generate_multiple_samples(model: MatchaTTS, text: str, depression_embedding, speaker_embedding, 
    ...
    for i, sample_temperature in enumerate(temps):
        try:
            out = synthesise_with_conditions(
                model, text, depression_embedding, speaker_embedding, 
...
def select_best_sample_by_similarity(samples: list, target_speaker_embedding: np.ndarray, 
                                   similarity_model: SpeakerSimilarityModel):
    """通过speaker similarity选择最佳样本（选择相似度最高的，无阈值限制）"""
    if not samples:
        return None, 0.0
    ...
    print(f"    🔍 评估 {len(samples)} 个候选样本的speaker similarity...")
    
    for sample in samples:
        try:
            # 从生成的音频中提取speaker embedding
            if similarity_model.use_reducer:
                generated_embedding = similarity_model.extract_speaker_embedding(
                    sample["waveform"], sr=22050
                )
            else:
                # 使用ECAPA原生空间
                generated_embedding = similarity_model.extract_raw_ecapa_embedding(
                    sample["waveform"], sr=22050
                )
            
            if generated_embedding is not None:
                # 计算与目标speaker embedding的相似度
                similarity = similarity_model.compute_similarity(
                    generated_embedding, target_speaker_embedding
                )
                similarities.append(similarity)
                
                print(f"      样本 {sample['sample_id']}: similarity = {similarity:.4f}")
                ...
                print(f"      ⚠️ 样本 {sample['sample_id']}: 无法提取speaker embedding")
    ...
    if best_sample is not None:
        print(f"    📌 选择最佳样本 {best_sample['sample_id']} (相似度: {best_similarity:.4f})")
...
@torch.inference_mode()
def generate_with_similarity_selection(model: MatchaTTS, text: str, depression_embedding, speaker_embedding,
    ...
    """
    采样式生成并基于 speaker similarity 选择最佳候选。
    - 当 SAMPLING_ENABLED=False 或 similarity_model 缺失时，退回单次合成。
    - 当 SIM_USE_ECAPA_192=True 且 target_speaker_embedding 为空时，退回单次合成（避免维度不一致）。
    - 返回 dict: {waveform, mel, temperature, sample_id, similarity, is_sampled}
    """
    # 选择模式：仅抑郁相似度
    if SELECT_BY_DEPRESSION_ONLY:
        if not SAMPLING_ENABLED:
            out = synthesise_with_conditions(model, text, depression_embedding, speaker_embedding)
            waveform = mel_to_waveform(out["mel"], vocoder, denoiser)
            return {
                "waveform": waveform,
                "mel": out["mel"],
    ...
    # 2) 不启用采样或无相似度模型 → 单次合成（说话人相似度路径）
    if (not SAMPLING_ENABLED) or (similarity_model is None):
        print("    📝 使用单次生成模式（采样未启用或相似度模型缺失）")
        out = synthesise_with_conditions(model, text, depression_embedding, speaker_embedding)
        waveform = mel_to_waveform(out["mel"], vocoder, denoiser)
        return {
            "waveform": waveform,
    ...
    # 3) 若使用 ECAPA-192 空间，但缺少目标参考嵌入 → 安全回退
    if SIM_USE_ECAPA_192 and (target_speaker_embedding is None):
        print("    ⚠️ 无 ECAPA 参考嵌入，退回单次生成以避免维度错配")
        out = synthesise_with_conditions(model, text, depression_embedding, speaker_embedding)
        waveform = mel_to_waveform(out["mel"], vocoder, denoiser)
        return {
            "waveform": waveform,
    ...
    print(f"    🎲 采样式生成 {num_samples} 个候选...")
    # 4) 生成多个候选（温度在 0.05~0.5）
    samples = generate_multiple_samples(
        model, text, depression_embedding, speaker_embedding,
        vocoder, denoiser, num_samples
    )
    ...
    # 5) 基于说话人相似度选择最佳候选
    best_sample, best_similarity = select_best_sample_by_similarity(
        samples, target_speaker_embedding, similarity_model
    )

    # 6) 自适应重采样：若最佳相似度低于阈值，则尝试更多样本
    if (USE_ADAPTIVE_SAMPLING and (best_sample is not None) and 
        (best_similarity < SIMILARITY_THRESHOLD_RETRY) and (MAX_RETRY_SAMPLES > 0)):
        print(f"    🔄 相似度 {best_similarity:.4f} 低于阈值 {SIMILARITY_THRESHOLD_RETRY}，进行额外采样...")
        additional = generate_multiple_samples(
            model, text, depression_embedding, speaker_embedding,
            vocoder, denoiser, MAX_RETRY_SAMPLES
        )
        if additional:
            add_best, add_sim = select_best_sample_by_similarity(
                additional, target_speaker_embedding, similarity_model
            )
            if add_best is not None and add_sim > best_similarity:
                print(f"    ✅ 额外采样找到更好样本：{add_sim:.4f} > {best_similarity:.4f}")
                best_sample, best_similarity = add_best, add_sim
            else:
                print(f"    ⚠️ 额外采样未找到更好样本：{add_sim:.4f} <= {best_similarity:.4f}")

    # 7) 最终兜底
    if best_sample is None:
        print("    ❌ 无法选择最佳样本，回退到第一个候选")
        best_sample = samples[0]
        best_similarity = 0.0
...
def main():
    ...
    # 加载embeddings
    (depression_embeddings, speaker_embeddings, depression_subject_ids, 
     speaker_subject_ids, depression_subject_to_idx, speaker_subject_to_idx) = load_embeddings()
    ...
    ecapa_refs = {}
    if SAMPLING_ENABLED and not SELECT_BY_DEPRESSION_ONLY:
        if not HAS_SPEECHBRAIN:
            print("✗ speechbrain library not available. Disabling sampling mode...")
            SAMPLING_ENABLED = False
        else:
            print("Loading Speaker Similarity Model...")
            try:
                similarity_model = SpeakerSimilarityModel(output_dim=32, use_reducer=not SIM_USE_ECAPA_192)
                print("✓ Speaker Similarity Model loaded successfully")
                # 若选择使用ECAPA原生192维空间，则为计划中的subjects构建参考嵌入
                if SIM_USE_ECAPA_192:
                    subj_ids = set(int(it["speaker_id"]) for it in generation_plan)
                    ecapa_refs = build_ecapa_reference_embeddings(
                        similarity_model,
                        subj_ids,
                        SAMPLING_REF_DIR,
                        max_audio_per_subject=MAX_REF_AUDIO_PER_SUBJECT
                    )
            except Exception as e:
                print(f"✗ Failed to load Speaker Similarity Model: {e}")
                print("Disabling sampling mode...")
                SAMPLING_ENABLED = False
                similarity_model = None
    elif SELECT_BY_DEPRESSION_ONLY:
        ...
        # 基于抑郁提取器构建“音频空间”的五级原型库（用于相似度目标）
        try:
            ref_dir = DEPRESSION_REF_AUDIO_DIR if DEPRESSION_REF_AUDIO_DIR else SAMPLING_REF_DIR
            subj_ids = sorted(set(int(it["depression_id"]) for it in generation_plan))
            dep_refs = build_depression_reference_embeddings(
                depression_extractor, set(subj_ids), ref_dir, max_audio_per_subject=MAX_REF_AUDIO_PER_SUBJECT
            )
            if len(dep_refs) >= 5:
                audio_subject_ids = np.array(sorted(dep_refs.keys()), dtype=int)
                audio_embeddings = np.stack([dep_refs[int(sid)] for sid in audio_subject_ids], axis=0)
                dep_bank_audio = build_dep_bank_clinical5(
                    audio_embeddings,
                    audio_subject_ids,
                    subject_to_score,
                    subject_to_phq8
                )
                print("✓ Audio-space Clinical-5 bank ready for similarity alignment")
            else:
                print("⚠️ Audio-space refs too few, skip audio-space bank")
        except Exception as e:
            dep_bank_audio = None
            print(f"⚠️ Failed to build audio-space bank: {e}")
    ...
    for subject_idx, item in enumerate(generation_plan):
        ...
        # 获取当前条目的speaker embedding（抑郁条件改为基于严重度的插值向量）
        ...
        with tqdm(total=len(subject_sentences), desc=f"Generating subject {subject_idx+1}/{len(generation_plan)} combo={combo_id}") as pbar:
            for _, sentence_text in enumerate(subject_sentences):
                try:
                    ...
                    # 相似度目标在“音频空间”（若可用）
                    dep_vec_target = dep_from_alpha_over_bank(dep_bank_audio, alpha) if dep_bank_audio is not None else None

                    # 仅抑郁相似度采样（此处以 dep_vec 作为条件）
                    if SAMPLING_ENABLED:
                        # 目标相似度统一使用插值 dep_vec（不使用 subject 参考嵌入）
                        best_sample = generate_with_similarity_selection(
                            model, sentence_text,
                            dep_vec, current_speaker_emb,
                            vocoder, denoiser, None,
                            target_speaker_embedding=None,
                            num_samples=NUM_SAMPLES,
                            depression_extractor=depression_extractor,
                            target_depression_ref=(dep_vec_target.detach().cpu().numpy() if torch.is_tensor(dep_vec_target) else dep_vec_target)
                        )
