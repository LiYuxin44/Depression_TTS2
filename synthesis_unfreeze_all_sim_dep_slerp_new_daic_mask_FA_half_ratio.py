...
OUTPUT_FOLDER_BASE = "/data/depression_tts/synthese_data/v7_dep_slerp_1111_7mask_3FA"

N_TIMESTEPS = 30
LENGTH_SCALE = 1.0
TEMPERATURE = 0  # 设为0以获得确定性输出（如需少量随机性可调到0.1~0.2）

# 🔥 生成配置 - 每个subject生成的句子数量（默认值，实际会被均衡逻辑覆盖）
SENTENCES_PER_SUBJECT = 50  

# Masking vs False Alarm 全局目标比例（在总体上希望 FA:Masking = 3:7）
MASKING_RATIO = 0.7   # 目标 masking 比例
FALSE_ALARM_RATIO = 0.3  # 目标 false alarm 比例
# False Alarm 使用的健康条件 alpha 原常数（现在不再强行用统一 -1，而是用各 subject 的 PHQ→alpha）
FALSE_ALARM_HEALTHY_ALPHA = -1.0  # 仍保留但不再直接用于 false_alarm 逻辑
    ...
    for csv_file in csv_files:
        try:
            if os.path.exists(csv_file):
                ...
                print(f"  Loaded PHQ from {csv_file}")
            else:
...
def compute_balanced_synthetic_targets(sev_orig: dict, bin_orig: dict, sev_subject_counts: dict = None) -> dict:
    ...
    sevmax_final_base = orig_counts.get(s_max, 0) + 2 * n_subj_smax
    ...
    print(f"  简化规则：S_max={s_max}, 原始={orig_counts.get(s_max,0)}, subjects={n_subj_smax}, S_max最终={final_target_same_each}")
...
def distribute_targets_to_plan(plan: list, needed_per_sev: dict, min_per_subject_per_sev: dict | None = None) -> list:
    """将每个严重度需要的合成条数分配到对应的 subjects 上。"""
    DEFAULT_MIN_UTTS = 1
...
def solve_conflict_ratios(total_dep, total_healthy,
                          target_mask_ratio=MASKING_RATIO,
                          target_false_ratio=FALSE_ALARM_RATIO):
    """
    解 r_mask, r_false，使得：
      (r_false * total_healthy) / (r_mask * total_dep) ≈ target_false_ratio / target_mask_ratio
    且 0 <= r_mask <= 1, 0 <= r_false <= 1
    r_mask 作用在抑郁 subject 的 target_sentences 上（用于 masking 占比）
    r_false 作用在健康 subject 的 target_sentences 上（用于 false_alarm 占比）
    """
    if total_dep == 0 or total_healthy == 0:
        # 极端情况，两侧不齐，干脆不生成冲突，全部走 no-conflict
        return 0.0, 0.0

    # 想要的比值： (FA / Masking) * (dep / healthy)
    desired = (target_false_ratio / target_mask_ratio) * (total_dep / total_healthy)

    # 先假设抑郁这边全部用来 masking（r_mask = 1），算健康这边需要多少比例
    r_mask = 1.0
    r_false = desired

    if r_false <= 1.0:
        return r_mask, r_false

    # 如果 r_false > 1，用不完，只能把健康这边全用来 false_alarm，再回头缩减 r_mask
    r_false = 1.0
    r_mask = (target_mask_ratio / target_false_ratio) * (total_healthy / total_dep)
    r_mask = min(1.0, r_mask)
    return r_mask, r_false
...
def load_embeddings():
    """加载 depression embeddings，并构建与训练一致的 subject→spk 映射。"""
...
def main():
    ...
    # ───────── 关键新增：根据总 target 计算全局冲突比例 r_mask / r_false  ─────────
    total_target_dep = sum(int(it.get("target_sentences", SENTENCES_PER_SUBJECT))
                           for it in generation_plan if int(it.get("class_label", 1)) == 1)
    total_target_healthy = sum(int(it.get("target_sentences", SENTENCES_PER_SUBJECT))
                               for it in generation_plan if int(it.get("class_label", 1)) == 0)
    r_mask, r_false = solve_conflict_ratios(total_target_dep, total_target_healthy)
    print(f"\n[CONF-PLAN] total_target_dep={total_target_dep}, total_target_healthy={total_target_healthy}")
    print(f"[CONF-PLAN] using r_mask={r_mask:.3f} (on depressed), r_false={r_false:.3f} (on healthy) for global FA:Mask ≈ {FALSE_ALARM_RATIO}:{MASKING_RATIO}")
    # ────────────────────────────────────────────────────────────────────────
    ...
    for subject_idx, item in enumerate(generation_plan):
        logical_subject_id = int(item["logical_subject_id"])
        depression_id = int(item["depression_id"])
        speaker_id = int(item["speaker_id"])
        class_label = int(item["class_label"])
        ...
        if not log_file.exists():
            with open(log_file, "w") as log:
                log.write("Combo_ID,Depression_ID,Speaker_ID,Pair_Type,PHQ8_Binary,Sentence_ID,Text,Audio_File,Status,Similarity_Score,Sample_ID,Is_Sampled,Generation_Type\n")
        ...
        if phq_score_current is not None:
            ...
            alpha_log = severity_to_alpha(sev)
        ...
        # ───────── 新逻辑：按标签决定可生成的冲突类型 + no-conflict ─────────
        is_dep = (class_label == 1)
        if is_dep:
            # 抑郁 subject：只能做 masking（抑郁声 + 正向文本），剩下用抑郁声 + 负向文本补
            num_masking = int(round(target_sentences * r_mask))
            num_false_alarm = 0
            num_no_conflict = target_sentences - num_masking
        else:
            # 健康 subject：只能做 false_alarm（健康声 + 负向文本），剩下用健康声 + 正向文本补
            num_false_alarm = int(round(target_sentences * r_false))
            num_masking = 0
            num_no_conflict = target_sentences - num_false_alarm

        num_masking = max(0, num_masking)
        num_false_alarm = max(0, num_false_alarm)
        num_no_conflict = max(0, num_no_conflict)

        print(f"  Severity: {sev}, Target sentences: {target_sentences}")
        print(f"    Masking (dep+pos): {num_masking}")
        print(f"    False Alarm (healthy+neg): {num_false_alarm}")
        print(f"    No-conflict: {num_no_conflict}  ({'dep+neg' if is_dep else 'healthy+pos'})")
        # ────────────────────────────────────────────────────────────────

        with tqdm(total=target_sentences, desc=f"Generating subject {subject_idx+1}/{len(generation_plan)} combo={combo_id}") as pbar:
            # 1) Masking（抑郁声 + 正向文本）——仅抑郁 subject 有
            for i in range(num_masking):
                try:
                    total_attempted += 1
                    ...
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",{audio_filename},Success,{similarity_score:.4f},{sample_id},{is_sampled},masking\n")
                    ...
                    print(f"Error generating masking sentence: {e}")
                    phq8_binary = subject_to_phq8.get(depression_id, 1)
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",N/A,Failed: {str(e)},0.0,-1,False,masking\n")
                    sentence_counter += 1
                    continue
            
            # 2) False Alarm（健康声 + 负向文本）——仅健康 subject 有
            for i in range(num_false_alarm):
                try:
                    total_attempted += 1
                    ...
                    phq8_binary = subject_to_phq8.get(depression_id, 1)
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",{audio_filename},Success,{similarity_score:.4f},{sample_id},{is_sampled},false_alarm\n")
                    ...
                    print(f"Error generating false_alarm sentence: {e}")
                    phq8_binary = subject_to_phq8.get(depression_id, 1)
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",N/A,Failed: {str(e)},0.0,-1,False,false_alarm\n")
                    sentence_counter += 1
                    continue

            # 3) No-conflict 样本：
            #    抑郁 subject: 抑郁声 + 消极文本 (no_conflict_dep)
            #    健康 subject: 健康声 + 积极文本 (no_conflict_healthy)
            for i in range(num_no_conflict):
                try:
                    total_attempted += 1

                    if is_dep:
                        pool_key = "depressed"
                        text_pool = depressed_text_pool
                        gen_type = "no_conflict_dep"
                    else:
                        pool_key = "healthy"
                        text_pool = healthy_text_pool
                        gen_type = "no_conflict_healthy"

                    idx = sev_text_pool_indices[sev][pool_key]
                    if idx >= len(text_pool):
                        idx = 0
                        sev_text_pool_indices[sev][pool_key] = 0
                    sentence_text = text_pool[idx]
                    sev_text_pool_indices[sev][pool_key] = idx + 1
                    ...
                    phq8_binary = subject_to_phq8.get(depression_id, 1)
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",{audio_filename},Success,{similarity_score:.4f},{sample_id},{is_sampled},{gen_type}\n")
                    ...
                    print(f"Error generating no_conflict sentence: {e}")
                    phq8_binary = subject_to_phq8.get(depression_id, 1)
                    with open(log_file, "a") as log:
                        log.write(f"{combo_id},{depression_id},{speaker_id},{pair_type},{phq8_binary},{sentence_counter},\"{sentence_text}\",N/A,Failed: {str(e)},0.0,-1,False,{gen_type}\n")
                    sentence_counter += 1
                    continue
        ...
        with open(stats_file, "w") as f:
            json.dump({
                ...
                "target_sentences": int(item.get("target_sentences", SENTENCES_PER_SUBJECT)),
                "masking_count": num_masking,
                "false_alarm_count": num_false_alarm,
                "no_conflict_count": num_no_conflict,
                "no_conflict_type": "dep+neg" if class_label == 1 else "healthy+pos",
