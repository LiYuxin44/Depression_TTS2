...
OUTPUT_FOLDER_BASE = "/data/depression_tts/synthese_data/v5_dep_slerp_A_1104"

LENGTH_SCALE = 1.0
N_TIMESTEPS = 48
TEMPERATURE = 0.15
TEMPERATURE_MIN = 0.12
...
CFG_UNCOND_MODE = "none"      # "none"→无条件分支用 depression_cond=None;
...
def synthesise_with_conditions(model: MatchaTTS, text: str, depression_embedding,
    ...
    if hasattr(model, "n_spks") and int(model.n_spks) > 1 and speaker_id is not None:
        try:
            spk_idx = GLOBAL_SUBJECT_TO_SPK_IDX[int(speaker_id)]
        except Exception:
    ...
    w = float(guidance_w) if (CFG_ENABLED and guidance_w is not None) else 0.0
    W_CAP = 3.5
    w = max(0.0, min(w, W_CAP))  # 建议不超过 1.5

    # ===== 真 CFG 路径（优先） =====
    if hasattr(model, "flow_sample_with_cfg") and callable(getattr(model, "flow_sample_with_cfg")) and w > 0.0:
        out = model.flow_sample_with_cfg(
            tex["x"], tex["x_lengths"],
            n_timesteps=n_steps, temperature=temperature,
            spks=spk_tensor, length_scale=length_scale,
            depression_cond=dep_cond, guidance_w=w,
        )
        out.update({**tex})
        return out
    ...
    dep_uncond = None if (uncond_mode or CFG_UNCOND_MODE or "none").lower() == "none" \
        else _make_uncond_depression_cond(dep_cond, dep_bank_for_uncond, (uncond_mode or CFG_UNCOND_MODE))

    out_c = model.synthesise(tex["x"], tex["x_lengths"], depression_cond=dep_cond, **synth_kwargs_base)
    out_u = model.synthesise(tex["x"], tex["x_lengths"], depression_cond=dep_uncond, **synth_kwargs_base) if dep_uncond is not dep_cond else out_c
    mel_guided = apply_cfg_on_mel(out_c["mel"], out_u["mel"], w)

    out = dict(out_c)
    out["mel"] = mel_guided
    out["cfg"] = {"enabled": True, "mode": "mel_fallback", "w": float(w)}
    out.update({**tex})
    return out
...
def main():
    ...
    # 固定：不使用命令行参数与显式配对，直接采用全交集 matched
    ...
    try:
        ...
        available_ids = sorted(list(set(depression_subject_to_idx.keys()) & set(speaker_subject_to_idx.keys()) & set(subject_to_phq8.keys())))
        if not available_ids:
            raise RuntimeError("没有找到可用的subject交集（或metadata缺标签）")
        ...
        generation_plan = []
        for it in base_plan:
            if DENSE_GRID_MODE == "alpha" and len(ALPHA_GRID) > 0:
                for a in ALPHA_GRID:
                    expanded = dict(it)
                    expanded["severity"] = "grid"
                    expanded["alpha"] = float(a)  # ∈ [0,1]
                    expanded["target_sentences"] = MONO_SENTENCES_PER_SEVERITY
                    generation_plan.append(expanded)
            elif DENSE_GRID_MODE == "w" and len(W_GRID) > 0:
                for wv in W_GRID:
                    expanded = dict(it)
                    expanded["severity"] = "grid"
                    expanded["w"] = float(wv)
    ...
    print(f"📊 单调验证计划（预览）")
    print(f"{'='*80}")
    print(f"  subjects: {len(set(it['logical_subject_id'] for it in generation_plan))}")
    print(f"  expanded items: {len(generation_plan)}")
    ...
    for subject_idx, item in enumerate(generation_plan):
        ...
        # 目录命名: 若 alpha 网格, 用 a_XX; 若 w 网格, 用 w_XX; 否则按严重度
        if DENSE_GRID_MODE == "alpha" and ("alpha" in item):
            sev_dir = f"a_{float(item['alpha']):.2f}"
        elif DENSE_GRID_MODE == "w" and ("w" in item):
            sev_dir = f"w_{float(item['w']):.2f}"
        else:
        ...
        # 为该条目计算 alpha+dep_vec（模型空间）
        sev = item.get("severity", None) or sev_log
        if DENSE_GRID_MODE == "alpha" and ("alpha" in item):
            # 将 [0,1] 的 alpha' 映射到 [-1,1]
            alpha = float(item["alpha"]) * 2.0 - 1.0
        else:
            alpha = severity_to_alpha(sev)
        dep_vec = dep_from_alpha_over_bank(dep_bank, alpha)

        print(f"  Subject combo={combo_id} embeddings:")
        print(f"    Depression embedding shape: {dep_vec.shape}")

        # 本 subject 的句子（共享句池）
        subject_sentences = list(global_sentences_pool)

        # guidance scale（按严重度分配）
        if DENSE_GRID_MODE == "w" and ("w" in item):
            guidance_w = float(item["w"]) if CFG_ENABLED else None
        else:
            guidance_w = severity_to_w(sev) if CFG_ENABLED else None

        # 温和放大 + 限幅：更“明显”但保持“真实”
        if guidance_w is not None:
            base_mult = 1.3  # 略降整体放大，减少过度风格化导致的失真
            if DENSE_GRID_MODE == "alpha" and ("alpha" in item):
                # alpha'∈[0,1] → 动态限幅：1.0 → 2.4，越抑郁允许越大
                a_prime = float(item["alpha"])  # 0..1
                cap = 1.1 + 1.4 * a_prime  # [1.1, 2.5]
            else:
                cap_by_sev = {
                    "normal": 0.8,
                    "mild": 1.2,
                    "moderate": 1.6,
                    "moderately_severe": 2.0,
                    "severe": 2.4,
                }
                cap = cap_by_sev.get(sev, 1.3)
            guidance_w = min(guidance_w * base_mult, cap)
            ...
            for _, sentence_text in enumerate(subject_sentences):
                try:
                    ...
                    out = synthesise_with_conditions(
                        ...
                        dep_vec, speaker_id,
                        guidance_w=guidance_w,
                        dep_bank_for_uncond=dep_bank,
                        uncond_mode=CFG_UNCOND_MODE
                    )
                    waveform = mel_to_waveform(out["mel"], vocoder, denoiser)
