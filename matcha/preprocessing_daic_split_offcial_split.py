...
output_wav_dir_test = "/data/test/processed_audio_test"
output_filelist = "/home/i-liyuxin/Depression_TTS/matcha/data/daic_filelist.txt"
sample_rate = 16000

# 读取验证集与测试集的 subject 列表
dev_split_path = "/home/i-liyuxin/Depression_TTS/matcha/data/dev_split_Depression_AVEC2017.csv"
test_split_path = "/home/i-liyuxin/Depression_TTS/matcha/data/full_test_split.csv"

val_subjects = []
test_subjects = []

if os.path.exists(dev_split_path):
    val_subjects = pd.read_csv(dev_split_path)["Participant_ID"].astype(str).tolist()

if os.path.exists(test_split_path):
    test_subjects = pd.read_csv(test_split_path)["Participant_ID"].astype(str).tolist()

os.makedirs(output_wav_dir, exist_ok=True)
os.makedirs(output_wav_dir_test, exist_ok=True)

# ====== 1. 读取所有参与者的 id ======
meta = pd.read_csv(metadata_path)
all_ids = meta["Participant_ID"].astype(str).tolist()

process_train_val = False
...
for pid in all_ids:
    if not process_train_val and pid not in test_subjects:
        continue
    ...
    for idx, row in df.iterrows():
        ...
        wav_dir = output_wav_dir_test if pid in test_subjects else output_wav_dir
        os.makedirs(wav_dir, exist_ok=True)
        wav_out = os.path.join(wav_dir, f"{pid}_{idx}.wav")
        torchaudio.save(wav_out, clip.unsqueeze(0), sample_rate)
        line = f"{wav_out}|{text}"
        
        # ======= 按 subject id 分组到 val/train =======
        if pid in test_subjects:
            test_lines.append(line)
        elif process_train_val and pid in val_subjects:
            val_lines.append(line)
        elif process_train_val:
...
test_file = output_filelist.replace(".txt", "_test.txt")
...
print(f"总样本: {len(train_lines) + len(val_lines) + len(test_lines)}, 训练: {len(train_lines)}, 验证: {len(val_lines)}, 测试: {len(test_lines)}")
if process_train_val:
    print(f"Train filelist: {train_file}")
    print(f"Val filelist: {val_file}")
print(f"Test filelist: {test_file}")
...
if process_train_val:
    ...
    with open(output_filelist.replace(".txt", "_val_subjects.txt"), "w") as f:
        for pid in set([l.split("/")[-1].split("_")[0] for l in val_lines]):
            f.write(pid + "\n")
with open(output_filelist.replace(".txt", "_test_subjects.txt"), "w") as f:
    for pid in set([l.split("/")[-1].split("_")[0] for l in test_lines]):
        f.write(pid + "\n")