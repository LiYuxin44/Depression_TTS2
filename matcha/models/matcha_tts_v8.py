...
from matcha.models.components.adapter import FiLMMuAdapter
...
class MatchaTTS(BaseLightningClass):  # 🍵
    def __init__(
        ...
        # 修改 decoder 参数（不再启用内部 Adapter 与解耦条件）
        decoder_params = dict(decoder)
        decoder_params.pop('use_adapter', None)
        decoder_params.pop('adapter_dim', None)
        decoder_params.pop('adapter_dropout', None)
        decoder_params.pop('adapter_cond_dim', None)
        decoder_params.pop('use_decoupled_conditions', None)
        decoder_params.pop('depression_cond_dim', None)
        decoder_params.pop('speaker_cond_dim', None)
        ...
        # 构建 Mu 级别的 FiLM 适配器（仅使用抑郁条件作为风格向量）
        self.mu_film_adapter = None
        #禁用film adapter
        if isinstance(self.depression_cond_dim, int) and self.depression_cond_dim > 0:
            self.mu_film_adapter = FiLMMuAdapter(
                channels=self.n_feats,
                style_dim=self.depression_cond_dim,
                mlp_hidden=adapter_dim,
    ...
    def synthesise(self, x, x_lengths, n_timesteps, temperature=1.0, spks=None, length_scale=1.0,
        ...
        # 使用 FiLM 适配器对 mu 做风格调制（仅抑郁条件）
        mu_for_decoder = mu_y
        if self.mu_film_adapter is not None and depression_cond_vec is not None:
            mu_for_decoder = self.mu_film_adapter(mu_y, depression_cond_vec)

        # Generate sample tracing the probability flow
        decoder_outputs = self.decoder(mu_for_decoder, y_mask, n_timesteps, temperature, spks)
    ...
    def forward(self, x, x_lengths, y, y_lengths, spks=None, out_size=None, cond=None, durations=None,
        ...
        diff_loss, _ = self.decoder.compute_loss(x1=y, mask=y_mask, mu=mu_for_decoder, spks=spks, cond=cond)
