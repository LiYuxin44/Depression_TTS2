...
SEVERITY_TO_FIXED_PHQ: Dict[str, int] = {
    "normal": 2,
    "mild": 7,
    "moderate": 12,
...
TRAIN_LABEL_CSV_DEFAULT = "/home/i-liyuxin/Depression_TTS/test/data/Label/train_cleaned.csv"


def phq_score_to_severity(score: float | int | None) -> str | None:
    if score is None:
        return None
    try:
        s = int(score)
    ...
    if 20 <= s <= 27:
        # CSV 中的 PHQ8_Score 实际范围可达 27，统一映射为 severe
        return "severe"
    return None


def load_train_labels(csv_path: Path) -> Tuple[Dict[int, float], Dict[int, str]]:
    if not csv_path.exists():
        raise FileNotFoundError(f"训练标签 CSV 不存在: {csv_path}")
    ...
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            if "Participant_ID" not in row:
                raise KeyError("CSV 缺少 Participant_ID 列")
            sid_raw = row.get("Participant_ID", "").strip()
            if not sid_raw:
                continue
            try:
                sid = int(sid_raw)
            ...
            score_raw = (row.get("PHQ8_Score") or "").strip()
            if score_raw == "":
                continue
            try:
                score_val = float(score_raw)
            except Exception:
                raise ValueError(f"无法解析 subject {sid} 的 PHQ8_Score: {score_raw}")

            sev = phq_score_to_severity(score_val)
            if sev is None:
                raise ValueError(f"subject {sid} 的 PHQ8_Score={score_val} 无法映射严重度")

            scores[sid] = score_val
            severities[sid] = sev

    if not scores:
        raise RuntimeError(f"未能从 CSV 载入任何 subject 的 PHQ8_Score: {csv_path}")

    return scores, severities


def find_depression_id(combo_dir: Path, combo_id: str) -> int:
    for json_file in sorted(combo_dir.glob("*.json")):
        try:
            with open(json_file, "r", encoding="utf-8") as f:
                data = json.load(f)
        except Exception as exc:
            raise RuntimeError(f"读取 {json_file} 失败: {exc}") from exc

        if isinstance(data, dict):
            dep_id = data.get("depression_id")
            if dep_id is None:
                continue
            try:
                dep_int = int(dep_id)
            except Exception:
                raise ValueError(f"{json_file} 中的 depression_id 无法转换为整数: {dep_id}")

            recorded_combo = data.get("combo_id")
            if recorded_combo is not None and recorded_combo != combo_id:
                continue
            return dep_int

    raise RuntimeError(f"目录 {combo_dir} 下未找到包含 depression_id 的 JSON 文件")


def write_p_label(wav_path: Path, phq_value: int) -> None:
    p_label_path = wav_path.with_suffix(".p_label")
    with open(p_label_path, "w", encoding="utf-8") as f:
        f.write(f"{phq_value}\n")


def backfill_labels(base_dir: Path, csv_path: Path) -> None:
    if not base_dir.exists() or not base_dir.is_dir():
        raise NotADirectoryError(f"backfill_dir 无效: {base_dir}")

    scores, severities = load_train_labels(csv_path)

    total_wavs = 0
    total_written = 0

    combo_dirs = sorted([p for p in base_dir.iterdir() if p.is_dir()])
    if not combo_dirs:
        raise RuntimeError(f"{base_dir} 下未找到任何组合目录")

    for combo_dir in combo_dirs:
        combo_id = combo_dir.name
        dep_id = find_depression_id(combo_dir, combo_id)

        if dep_id not in scores:
            raise KeyError(f"CSV 中未找到 depression_id={dep_id} 的分数（组合 {combo_id}）")

        severity = severities.get(dep_id)
        if severity is None:
            raise KeyError(f"无法为 depression_id={dep_id} 映射严重度（组合 {combo_id}）")

        fixed_phq = SEVERITY_TO_FIXED_PHQ.get(severity.lower())
        if fixed_phq is None:
            raise KeyError(f"严重度 {severity} 未在固定映射中定义")

        wav_files = sorted(combo_dir.glob(f"{combo_id}_*.wav"))
        if not wav_files:
            raise RuntimeError(f"组合 {combo_id} 未找到任何 wav 文件")

        for wav_path in wav_files:
            total_wavs += 1
            write_p_label(wav_path, fixed_phq)
            total_written += 1

        print(f"✓ 组合 {combo_id} (dep={dep_id}) 已写入 {len(wav_files)} 个 .p_label -> {fixed_phq}")

    print(f"完成！总计处理 {total_wavs} 个 wav，写入 {total_written} 个 .p_label")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="为已生成的语音补写 PHQ 值标签 (.p_label)")
    parser.add_argument(
        "--backfill_dir",
        type=str,
        required=True,
        help="需要补写标签的根目录，例如 /data/depression_tts/synthese_data/v6_dep_slerp_1110_01",
    )
    parser.add_argument(
        "--train_label_csv",
        type=str,
        default=TRAIN_LABEL_CSV_DEFAULT,
        help="包含 PHQ8_Score 的训练标签 CSV 路径（默认: %(default)s）",
    )
...
def main() -> None:
    args = parse_args()
    backfill_dir = Path(args.backfill_dir).resolve()
    csv_path = Path(args.train_label_csv).resolve()

    backfill_labels(backfill_dir, csv_path)
...
# 统一为每个严重度分配固定的 PHQ 分数（用于生成 .p_label 文件）
SEVERITY_TO_FIXED_PHQ = {
    "normal": 2,
    "mild": 7,
    "moderate": 12,
    "moderately_severe": 17,
...
def _compose_combo_id(speaker_id: int, depression_id: int) -> str:
    """组合ID = speaker_id + depression_id（无分隔符，字符串拼接）"""
    return f"{int(speaker_id)}{int(depression_id)}"

def _find_depression_id_for_combo(combo_dir: Path,
                                  combo_id: str,
                                  plan_detail_path: Path | None) -> int | None:
    """尝试获取该组合对应的 depression_id，用于查找 PHQ 分数。"""
    # 1) 组合级 stats
    try:
        for p in combo_dir.glob("*.json"):
            try:
                with open(p, "r", encoding="utf-8") as f:
                    data = json.load(f)
                if isinstance(data, dict):
                    if data.get("combo_id") == combo_id and "depression_id" in data:
                        dep_id = data.get("depression_id")
    ...
    # 2) 详细计划
    try:
        if plan_detail_path and plan_detail_path.is_file():
            with open(plan_detail_path, "r", encoding="utf-8") as f:
                plan = json.load(f)
            gen_plan = plan.get("generation_plan", [])
            if isinstance(gen_plan, list):
                for item in gen_plan:
                    try:
                        spk = item.get("speaker_id")
                        dep = item.get("depression_id")
                        if spk is None or dep is None:
                            continue
                        if _compose_combo_id(spk, dep) == combo_id:
                            return int(dep)
                    except Exception:
                        continue
    except Exception:
...
def _load_phq_from_train_csv(csv_path: Path) -> tuple[dict[int, float], dict[int, str]]:
    """从训练标签CSV读取 PHQ 分数并转换为严重度。"""
    phq_scores: dict[int, float] = {}
    phq_severity: dict[int, str] = {}
    if not csv_path.exists():
        print(f"⚠️ 训练标签CSV未找到: {csv_path}")
        return phq_scores, phq_severity
    ...
    try:
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                try:
                    sid = int(row.get("Participant_ID"))
                except Exception:
                    continue
                score_raw = row.get("PHQ8_Score")
                ...
                try:
                    ...
                    continue
                phq_scores[sid] = score_val
                sev = phq_score_to_severity(score_val)
                if sev is not None:
                    phq_severity[sid] = sev
        print(f"✓ 从CSV载入 {len(phq_scores)} 个 subject 的 PHQ 分数 (来源: {csv_path})")
    except Exception as e:
        print(f"✗ 读取训练标签CSV失败: {e}")
    return phq_scores, phq_severity

def backfill_p_labels(base_dir: str, train_label_csv: str | None = None):
    """为已有的数据目录补写 .p_label 文件（按严重度写固定PHQ分数）"""
    root = Path(base_dir)
    if not root.exists() or not root.is_dir():
        print(f"✗ backfill_dir 不存在或不是目录: {base_dir}")
        return

    csv_path = Path(train_label_csv) if train_label_csv else Path(TRAIN_LABEL_CSV_DEFAULT)
    phq_scores, phq_severity = _load_phq_from_train_csv(csv_path)

    # 详细计划文件（若存在则可提供严重度信息）
    plan_detail = root / "generation_plan_detailed.json"

    print(f"开始补写 .p_label 到目录: {root}")
    print(f"计划文件: {plan_detail if plan_detail.exists() else '未找到，将尝试从各组合stats读取'}")

    num_wavs = 0
    num_written = 0
    num_skipped = 0

    for combo_dir in sorted([p for p in root.iterdir() if p.is_dir()]):
        combo_id = combo_dir.name
        dep_id = _find_depression_id_for_combo(combo_dir, combo_id, plan_detail)

        sev = None
        if dep_id is not None:
            sev = phq_severity.get(dep_id)
            if sev is None:
                score_val = phq_scores.get(dep_id)
                if score_val is not None:
                    sev = phq_score_to_severity(score_val)

        sev_key = str(sev).lower() if isinstance(sev, str) else None
        fixed_score = SEVERITY_TO_FIXED_PHQ.get(sev_key) if sev_key else None

        if fixed_score is None:
            # 兜底 moderate
            fixed_score = SEVERITY_TO_FIXED_PHQ.get("moderate")
            try:
                if dep_id is None:
                    print(f"  [WARN] 组合 {combo_id} 未能确定 depression_id，使用默认 moderate={fixed_score}")
                else:
                    print(f"  [WARN] 组合 {combo_id} (dep={dep_id}) 未能匹配严重度，使用默认 moderate={fixed_score}")
            except Exception:
                pass
        else:
            try:
                print(f"  ✓ 组合 {combo_id} (dep={dep_id}) 映射严重度 {sev} -> PHQ {fixed_score}")
            except Exception:
                pass

        # 遍历该组合下的 wav
        for wav_path in combo_dir.glob(f"{combo_id}_*.wav"):
            num_wavs += 1
            p_label = wav_path.with_suffix(".p_label")
            if p_label.exists():
                num_skipped += 1
                continue
            try:
                with open(p_label, "w") as f:
                    f.write(f"{int(fixed_score)}\n")
                num_written += 1
            except Exception as e:
                print(f"  ✗ 写入失败: {p_label} - {e}")

    print(f"完成补写: 共发现 {num_wavs} 个 wav, 新写入 {num_written} 个 .p_label, 已存在跳过 {num_skipped} 个")
...
def parse_args():
    ...
    parser.add_argument("--backfill_dir", type=str, default=None,
                        help="对已生成的数据进行补标：为每个 .wav 生成 .p_label（不做音频合成）。填写输出基础目录，例如 /data/depression_tts/synthese_data/v6_dep_slerp_1110_01")
    parser.add_argument("--train_label_csv", type=str, default=None,
                        help="补标时使用的训练标签CSV路径（默认为 test/data/Label/train_cleaned.csv）")
    return parser.parse_args()
...
def save_audio_and_label(subject_id: str,
                         sentence_id: int,
                         waveform,
                         folder: str | Path,
                         subject_to_phq8: dict,
                         label_subject_id: int | None = None,
                         severity: str | None = None):
    """保存音频文件、二分类label以及PHQ分数label
    ...
    if severity is not None:
        phq_score = SEVERITY_TO_FIXED_PHQ.get(str(severity).lower())

    if phq_score is None and label_sid is not None:
        # 若未提供或无法识别 severity，则退回已加载的原始 PHQ 分数
        phq_score = SUBJECT_PHQ_SCORES.get(label_sid)

    if phq_score is None:
        # 最终兜底：写入 moderate 对应的固定分数并提示
        phq_score = SEVERITY_TO_FIXED_PHQ.get("moderate")
        try:
            print(f"Warning: 未能确定 {subject_id}_{sentence_id} 的PHQ分数，使用默认值 {phq_score}")
        except Exception:
            pass

    p_label_filename = f"{subject_id}_{sentence_id}.p_label"
    with open(folder / p_label_filename, "w") as f:
        f.write(f"{int(phq_score)}\n")
...
def main():
    ...
    # 解析命令行参数
    args = parse_args()
    # 若指定 backfill，仅进行补标后退出
    if args.backfill_dir is not None and str(args.backfill_dir).strip():
        backfill_p_labels(args.backfill_dir, args.train_label_csv)
        return
        ...
        with tqdm(total=target_sentences, desc=f"Generating subject {subject_idx+1}/{len(generation_plan)} combo={combo_id}") as pbar:
            ...
            for i in range(num_healthy):
                try:
                    ...
                    audio_filename = save_audio_and_label(
                        combo_id,
                        sentence_counter,
                        waveform,
                        output_folder,
                        subject_to_phq8,
                        label_subject_id=depression_id,
                        severity=sev,
                    )
