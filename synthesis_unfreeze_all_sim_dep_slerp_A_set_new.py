...
N_TIMESTEPS = 30
LENGTH_SCALE = 1.0
TEMPERATURE = 0.7  # 设为0以获得确定性输出（如需少量随机性可调到0.1~0.2）
...
def synthesise_with_conditions(model: MatchaTTS, text: str, depression_embedding, speaker_id: int | None, temperature_override=None):
    ...
    if depression_cond is not None:
        # 只做 L2 归一化，不去中心化（保留不同严重度之间的相对位置）
        # 注意：如果训练时使用了去中心化，这里也需要保持一致
        depression_cond = _l2norm(depression_cond)
        # 原来的代码：depression_cond = _l2norm(depression_cond - depression_cond.mean())
        # 问题：减去单个 embedding 的均值会破坏不同严重度之间的相对位置
...
def generate_multiple_samples(model: MatchaTTS, text: str, depression_embedding, speaker_id: int | None, 
    ...
    """生成多个采样候选：温度由 TEMPERATURE_MIN/MAX 控制，并包含全局 TEMPERATURE。"""
    samples = []
    if num_samples <= 0:
        num_samples = 1
    ...
    if SAMPLING_ENABLED:
        ...
        try:
            ...
            if len(dep_refs) >= 5:
                ...
                try:
                    print_bank_diagnostics(dep_bank_audio, name="audio-space")
                except Exception:
                    pass
                bank_alignment_metrics = summarize_bank_alignment(dep_bank, dep_bank_audio)
            else:
        ...
        # 强制要求音频空间对齐（可配置）
        if REQUIRE_AUDIO_SPACE_BANK and (depression_extractor is None or dep_bank_audio is None):
            raise RuntimeError("Audio-space bank is required but not available. Check DEPRESSION_AUDIO_MODEL_CKPT and refs.")
    ...
    for subject_idx, item in enumerate(generation_plan):
        ...
        with open(stats_file, "w") as f:
            json.dump({
                ...
                "prototype_alignment": bank_alignment_metrics
            }, f, indent=2)
