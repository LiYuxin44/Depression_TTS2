...
async def process_single_text_for_sentiment_async(text_info: tuple, client: AsyncOpenAI, 
    ...
    try:
        ...
        # 判断情绪类别
        is_positive = ("积极" in sentiment_result or "positive" in sentiment_result.lower())
        is_negative = ("消极" in sentiment_result or "negative" in sentiment_result.lower())
        
        category = None
        if is_positive:
            category = "healthy"  # 积极文本 -> Healthy_Text_Pool
        elif is_negative:
...
async def analyze_sentiment_pool_async():
    ...
    print(f"\n去重后:")
    print(f"  Healthy_Text_Pool (积极文本): {len(healthy_texts)} texts")
